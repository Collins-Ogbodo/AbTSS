"""Cantilever Environment for Reinforcement Learning
This environment simulates a cantilever beam and allows for optimal sensor placement using reinforcement learning techniques.
It is designed to be used with OpenAI Gym and provides a multi-discrete action space for selecting sensor locations and directions.
The environment includes functionalities for resetting the state, taking actions, and calculating rewards based on the Fisher Information Matrix.
It also supports adversarial training scenarios where an adversary can modify the environment to challenge the agent's policy.
...
class CantileverEnv_v0_1(gym.Env):
    """
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.max_step = config.get("eps_length",1000)
        self.eps_length = self.max_step
        # Set up action and observation spaces
        self.action_space = gym.spaces.MultiDiscrete([self.num_sensors, len(self.ACTIONS)])
        num_nodes = 1462
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(num_nodes + len(config.get('sim_modes')),), 
                                                dtype=np.int8)

        # Initialize PyAnsys environment and state variables
        self.pyansys_env = config.get('pyansys_env')
        self.observation_space_node = self.pyansys_env["observation_space_node"]
        self.coord_2d_array = self.pyansys_env["coord_2d_array"]
        ...
        #mode shape initilisation
        self.phi_index = config.get("phi_index", (0,))

        ## Initial state from heuristic placement
        self.all_init_state = np.array([2356, 2347, 2339, 2332, 2327, 2321, 2314, 2307, 2301, 2295, 2288, 2279])
        indices = np.linspace(0, len(self.all_init_state) - 1, self.num_sensors, dtype=int)
        init_state = self.all_init_state[indices]
        self.initialize_state =  np.array(init_state)
        #Sensor mutation initialisation space
        #self.mutation_init_space =  [x for x in self.all_init_state if x not in remove_set]
        self.mutation_init_space = self.observation_space_node.copy()
        self.mutation_init_space = np.setdiff1d(self.mutation_init_space, self.initialize_state, assume_unique=True)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        try:
            ...
            self.current_state_binary = np.isin(self.observation_space_node, self.current_state).astype(np.int8)
            ...
            return np.concatenate((self.current_state_binary, self.bin_encode)), reward, done, info
        
        except Exception as e:
            logging.error(f"Error during step execution: {e}")
            raise

    def _reset_env(self, seed: Optional[int] = None, options: Optional[dict] = None, 
                   edit_sensors = None, edit_sensor_ini_loc = None):
        """
        ...
        try:
            ...
            self.phi = self.phi_data[self.phi_index]
            self.correlation_covariance_matrix = self.correlation_covariance_matrix_data[self.phi_index]
            #Binary encoding
            self.bin_encode = self.encode_data[self.phi_index]
            #State initilisation
            self.current_state = self.initialize_state.copy() #self._initialize_state()
            if isinstance(edit_sensor_ini_loc, np.ndarray)  and isinstance(edit_sensors, np.ndarray):
                self.current_state[edit_sensors] = edit_sensor_ini_loc
    ...
    def _info_metric(self, state: np.ndarray) -> float:
        ...
        try:
            ...
            L_mat = (self.observation_space_node[:, None] == state).astype(np.int8).T
    ...
    def _action_state_map(self, state: np.ndarray, action: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            state_row = self.coord_2d_array[self.coord_2d_array[:, 0] == state_node_id, 1:][0]
            ...
            distances = np.linalg.norm(self.coord_2d_array[:, 1:] - new_state_coord, axis=1)
            state[sensor_id] = self.coord_2d_array[np.argmin(distances), 0]
            return state
        except IndexError:
    ...
    def render(self, mode='human'):
        """
        Rendering of environment not available during training
        """
        ##TODO add the rendering of the environment for policy evaluation
        pass