"""Ansys Cantilever Environment
This module provides an environment for simulating a cantilever beam using Ansys Mechanical through the PyMechanical API.
It sets up the cantilever geometry, runs modal analysis to obtain mode shapes, mesh information and provides functionalities 
for sensor placement and rendering. 
"""
import json
...
class Cantilever():
    """  RL agent evironment for a Cantilever
        ...
        Input
        episode : number of configurations
        num_sensors : number of sensors to be placed
        sim_modes : list of mode shapes to be simulated
        norm : boolean, if True then normalize the mode shape matrix    
    """
    def __init__(self, config: dict) -> None:
        ...
        self.geo_path = os.path.join(script_dir,"Geometry","Cantilever-EMA.agdb")
        ...
        self.mode_shape_folder_name =  os.path.join(script_dir,"Mode_Shape")
...
ExtAPI.SelectionManager.NewSelection(my_selection)
        ...
        #Sequential combination of mode shapes
        num_mode = config.get('sim_modes')
        n = len(num_mode)
        self.mode_shape_comb = [tuple(num_mode[i:i+l])
        ...
        #precompute matrices
        self.phi_data = {}
        self.correlation_covariance_matrix_data = {}
        self.encode_data = {}
        encode_list = mlb.fit_transform(self.mode_shape_comb)
        for comb, encode in zip(self.mode_shape_comb, encode_list):
            phi = self.phi[:, comb]
            self.phi_data[comb] = phi
            self.encode_data[comb] = np.array(encode)
            self.correlation_covariance_matrix_data[comb] = correlation_covariance_matrix(phi)
