...
#Import packages from folder
from envs.cantilever import *
from envs.cantilever.adversarial import *
from envs.runners.adversarial_runner import AdversarialRunner 
from util import make_agent, FileWriter, safe_checkpoint, create_parallel_env, make_plr_args, envs_params
...
if __name__ == '__main__':
    ...
    progress_bar = tqdm(
    range(initial_update_count, num_updates),
    ...
    for j in progress_bar:
        ...
        if log:
            ...
            stats.update(test_stats) # Ensures sps column is always before test stats
                    # Update tqdm description with dynamic stats
            progress_bar.set_postfix(
            {"sps" : f"{sps:.2f}",
            "reward" : f"{stats.get('mean_env_return', 0):.3f}"}
            )
            log_stats(stats)
